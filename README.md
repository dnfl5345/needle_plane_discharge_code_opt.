...
| 7 | **적응형 반암시적 포아송**(Ventzek–Hagelaar): 유전완화 한계가 다른 한계보다 'LU 재분해 비용비' 이상 작을 때만 `∇·[(ε₀ε_r+dt·σ)∇V]=−ρ` 로 재조립·재분해(σ 변화 10 % 이내면 분해 재사용) | 도전 채널 단계에서 dt 0.04 ps → 0.86 ps, 벽시계 3.9배 단축 (§3.3) |
...
| 항목 | 값 |
|---|---|
| 갭 브리징 | **t = 8.06 ns** (평균 진전속도 ≈ 1.3 mm/ns, 평판 근처 5 mm/ns) |
| 방전 단계 서브스텝 / 벽시계 | 8,058 / **51 s** (6.3 ms/서브스텝, 포아송 27 %) |
| 잔광 10 µs | < 1 s |
| **총 소요** | **56 s** (v17: 브리징까지만 헤드리스 35 분·GUI ≈ 2 시간, 그 이후 진행 불가) |
| 최종 max nₑ / max \|E\| | 1.5×10²¹ m⁻³ / 170 kV/cm |
| 잔광 10 µs 후 활성종 총개수 | O(P) 2.8×10¹⁵, O₃ 3.1×10¹⁵, NO 1.1×10¹⁴, N(S) 1.5×10¹⁴ |
...
### 4.2 직렬저항 회로로 임펄스 전 구간 (R = 1 kΩ, 0–300 ns + 잔광 10 µs) — `results/run_R1000/`
RUN_R1000_PLACEHOLDER

### 4.3 격자 세분 (nx = 321, h = 0.0625 mm, 미지수 7.3만) — `results/run_default_nx321/`
RUN_NX321_PLACEHOLDER
...
| 초기화 | 6.5 s | 0.1 s |
| 브리징까지 서브스텝 | 28,794 | 8,058 |
| 브리징까지 벽시계(헤드리스) | ≈ 35 min | 51 s |
| 브리징 이후 | dt 5 fs 로 붕괴, 진행 불가 | 자동 감지 → 잔광, 또는 R>0 회로로 300 ns 완주 |
